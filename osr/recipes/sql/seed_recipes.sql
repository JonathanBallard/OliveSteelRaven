...
--
-- Requires:
--   psql -v owner_id=1 -f seed_recipes.sql <dbname>
-- ==========================================================
...
ingredient_upsert AS (
  ...
  SELECT
    initcap(regexp_replace(trim(name), '\s+', ' ', 'g')),
...
WHERE NOT EXISTS (
  ...
  WHERE x.recipe_id = rec.id
    AND x.line_order = ri.line_order
);
